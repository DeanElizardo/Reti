...
class TestQueen(unittest.TestCase):
    def queen_factory(self, color: str, space: Space) -> Queen:
        return Queen(color, space)
    
    def make_white_queen(self, space: Space) -> Queen:
        return self.queen_factory(WHITE, space)
    
    def make_black_queen(self, space: Space) -> Queen:
        return self.queen_factory(BLACK, space)
    ...
    def white_queen_has_right_moves(self, case: CaseData):
        queen = self.make_white_queen(case.init_space)
        actual_spaces: list[Space] = []
        for m in queen.moves:
            for s in m:
    ...
    def test_white_queen(self):
        ...
        for init_space in all_spaces:
            ...
            with self.subTest(f"expected {expected_str[:-1]}", init_space=init_space):
                self.white_queen_has_right_moves(test_case)

    def black_queen_has_right_moves(self, case: CaseData):
        queen = self.make_black_queen(case.init_space)
        actual_spaces: list[Space] = []
        for m in queen.moves:
            for s in m:
                actual_spaces.append(s)
    ...
    def test_black_queen(self):
        ...
        for init_space in all_spaces:
            ...
            test_case: CaseData = CaseData(init_space, expected_spaces)
            with self.subTest(f"expected {expected_str[:-1]}", init_space=init_space):
                self.black_queen_has_right_moves(test_case)
